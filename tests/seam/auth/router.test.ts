...
import type { StepUpFactor } from "../../../src/auth/app/types";
import { authPageRouteMap } from "../../../src/auth/routes";
...
async function claimEnv(verified = true) {
  ...
  return { env: { ...env, AUTH_KV: kv, DB: db } as unknown as Env, kv, db };
}
...
function confirm(entry: typeof app, env: Env, token?: string): Promise<Response> {
  ...
  return entry.request(`${authPageRouteMap.authEmailConfirm.href()}${query}`, { headers: CALLER }, env);
}
...
describe("the second-factor configuration this app ships", () => {
  it("sends a signed-in visitor owing the mandatory factor to the page that enrols it", async () => {
    ...
    const res = await app.request(authPageRouteMap.account.href(), { headers: { cookie } }, env);
  ...
  it("sends an anonymous visitor to sign in, carrying the page it was asked for", async () => {
    const { env } = authEnv();
    const res = await app.request(authPageRouteMap.account.href(), {}, env);
    expect(res.status).toBe(302);
  ...
  it("names the mandatory factor as the enrolment a settled visitor is bounced from", async () => {
    ...
    expect(res.status).toBe(303);
    expect(res.headers.get("location")).toBe(authPageRouteMap.account.href());
  });
});
...
describe("the auth group's pipeline on a sign-in POST", () => {
  const SPRITE = assets.path("svg/sprite.svg");

  /** The one refusal the sign-in page renders under its address field, whole. */
  function fieldRefusal(message: string): string {
    return `<p data-slot="field-error" class="text-sm font-normal text-destructive-text" id="field-email-error" role="alert"><svg data-slot="icon" viewBox="0 0 24 24" class="me-2 inline-block size-4" aria-hidden="true"><use href="${SPRITE}#icon-alert"></use></svg>${message}</p>`;
  }

  /** A sign-in posted under a fresh anonymous session with a valid token, on `entry`, carrying `headers` in place of the caller's. */
  async function signin(env: Env, body: URLSearchParams, headers: Record<string, string> = BROWSER_POST, entry: typeof app = app) {
    const { cookie, id } = await anonymous(env.AUTH_KV as unknown as ReturnType<typeof fakeKV>);
    const token = await mintTestCsrfToken(CSRF_SECRET, SIGNIN_PATH, { subject: id });
    return entry.request(SIGNIN_PATH, { method: "POST", headers: { ...headers, cookie, "X-CSRF-Token": token }, body }, env);
  }

  function withoutLimiter(): Env {
    const { RATE_LIMITER: _absent, ...env } = authEnv().env as unknown as Record<string, unknown>;
    return env as unknown as Env;
  }

  it("answers 503 on the production entry when the RATE_LIMITER binding is absent", async () => {
    const res = await signin(withoutLimiter(), new URLSearchParams({ email: USER_EMAIL }));

    expect(res.status).toBe(503);
    expect(await res.text()).toBe("Service unavailable");
  });

  it("admits the sign-in on the development entry with the binding absent, which is the allowance's whole grant", async () => {
    const dev = createWorker(securityHeaders, devAllowance({ rateLimitOptional: true }));
    const res = await signin(withoutLimiter(), new URLSearchParams({ email: USER_EMAIL }), BROWSER_POST, dev);

    expect(res.status).toBe(303);
    expect(res.headers.get("location")).toBe("/auth/verify");
  });

  it("refuses an undeclared field with 422 and the page's generic refusal", async () => {
    const res = await signin(authEnv().env, new URLSearchParams({ email: USER_EMAIL, role: "admin" }));

    expect(res.status).toBe(422);
    expect(elementOf(await res.text(), "p", 'data-slot="field-error"')).toBe(
      fieldRefusal("We could not read that. Please check the form and try again."),
    );
  });

  it("refuses a duplicated email key with 422 rather than taking either address", async () => {
    const body = new URLSearchParams();
    body.append("email", USER_EMAIL);
    body.append("email", UNKNOWN_EMAIL);
    const res = await signin(authEnv().env, body);

    expect(res.status).toBe(422);
    expect(elementOf(await res.text(), "p", 'data-slot="field-error"')).toBe(fieldRefusal("Enter an email address in the form name@example.com."));
  });

  it("refuses an inherited name with 422 and the page's generic refusal, polluting nothing", async () => {
    const body = new URLSearchParams({ email: USER_EMAIL });
