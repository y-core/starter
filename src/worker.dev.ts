import { devAllowance, type DevAllowanceOptions } from "@y-core/forge/dev";
...
const GRANTS: DevAllowanceOptions = { errorDetail: true };
GRANTS.rateLimitOptional = true; /* feature:rate-limit */
GRANTS.turnstileTestingSecrets = true; /* feature:turnstile */

/** The dev entry, and the only module here importing `@y-core/forge/dev` at value. */
export const devApp = createWorker(mergeSecurityHeaders(securityHeaders, { scriptSrc: [WRANGLER_LIVE_RELOAD_HASH] }), devAllowance(GRANTS));

// The production worker's own wrapper rather than a copy of it, so dev and production cannot drift
// apart and every entry point is exercised by the entry development actually runs.
