...
The two `!` rules cover everything `icons.outputs` writes. Cloudflare serves those directly,
bypassing the Worker — faster, and no CPU spent on a `fetch` event for a file that never needs
dynamic logic.

**Two rules, and neither grows when the icon set does.** `/static` is the `publicPrefix` on the
`icons` block in `config/assets.ts`, so every output lands beneath it and is covered by one glob.
`favicon.ico` is the single output marked `root: true`, because a browser probes `/favicon.ico`
whenever there is no HTML head to read — a PDF, image, JSON or download tab, and a
platform-generated error page — as do unfurlers and feed readers that never parse HTML.

Every other path, `/assets/*` included, runs the Worker first. The `validate-asset-root` gate row
diffs the two halves, so an output escaping the prefix without a rule here is caught in CI rather
than in production. See CONFIGURATION_AND_SECRETS.md §4a.

### 5c. Asset Fingerprinting and Cache Rules

`bun run build:assets` passes `--minify`, which content-hashes everything under `public/assets`
(`css/main.84b72c2e.css`). `layout.tsx` never names a hashed file: it asks the generated manifest
via `assets.path("css/main.css")`, so a new hash needs no edit anywhere.

`public/_headers` is generated by the same build, and the two halves of the tree earn different
rules:

| Path              | Cache-Control                                          | Why                                       |
| ----------------- | ------------------------------------------------------ | ----------------------------------------- |
| `/assets/*`       | `public, max-age=31536000, immutable`                  | content-hashed — a change is a new URL    |
| each icon         | `public, max-age=86400, stale-while-revalidate=604800` | stable filename — must stay bustable      |
| `/static/site.webmanifest` | `public, max-age=0, must-revalidate`          | how an installed app sees a changed icon set |

The icon rules are emitted **one per file, not as a `/static/*` glob**: Cloudflare applies every
`_headers` rule a request matches and joins a repeated header with a comma, so an overlapping glob
would hand the manifest a `Cache-Control` carrying both values.

`_headers` is a build output — `public/` is gitignored and rewritten on every build, so a rule is
changed in forge's `emitHeaders`, never in the file.
...
| -------------- | ------------- | ----------------------------------------------------------------------------------- |
| `LOGS_KV`      | KV Namespace  | Structured log storage — optional, degrades to console                              |
| `AUTH_KV`      | KV Namespace  | Sessions — **required**                                                             |
| `AUTH_DB`      | D1 Database   | Identity: users, factors, credentials, OTP state, challenges, nonces — **required** |
| `RATE_LIMITER` | Rate Limit    | Per-IP request throttling — optional, no-op when absent                             |
